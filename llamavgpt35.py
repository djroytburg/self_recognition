...
def generate_gpt_logprob_results(
    ...
    responses, articles, keys = load_data(dataset, sources = SOURCES + [TARGET], extras=True)
...
for model in [TARGET]:
    for dataset in ["cnn", "xsum"]:
        ...
        if "compare" in sys.argv[-1]:
            ...
            save_to_json(results, f"individual_setting/score_results/{dataset}/{model}_comparison_results{'_' + str(N) if N != 1000 else ''}_extra.json")
    print(model)
